	...
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper.select(Qry, function(result) {
			if (result.array != undefined) {
				...
				data["TOTBAD"]=eval(result[0][33])-eval(result[0][60]).toFixed(decimalplace);
...
function getCollectionInfo() {
	data = {};
	var Qry = "SELECT cm.customercode,customername,customeraddress1 as address,typecode,CAST(amount AS VARCHAR) amount,bankname,checkdate,checknumber,invoicenumber,comments,printstatus,(SELECT CASE cm.messagekey5 WHEN 0 THEN '' ELSE messageline1 || messageline2 || messageline3 || messageline4 END FROM customermessages WHERE messagekey = messagekey5) AS invheadermsg,IFNULL(excesspayment,'') AS excesspayment,cm.alternatecode FROM arheader arh JOIN customermaster cm ON cm.customercode = arh.customercode JOIN cashcheckdetail ccd ON ccd.visitkey = arh.visitkey LEFT JOIN bankmaster bm ON bm.bankcode = ccd.bankcode WHERE arh.visitkey = "
			+ sessionStorage.getItem("VisitKey");
	console.log(Qry);
	if (platform == 'iPad') {
		Cordova.exec(function(result) {
