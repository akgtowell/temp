...
function getbatchexpirydetail()
{
    ...
    {
                window.plugins.DataBaseHelper.select("select routekey,batchdetailkey,batchnumber,itemcode,expirydate,quantity,transactiontypecode,visitkey from batchexpirydetail where issync=0 and istemp='false and routekey in (" + routekey + ") and visitkey in (" + visitkey + ")'",function(result)
                {
                            if(!$.isEmptyObject(result))
                            {
                                    for(i=0;i<result.array.length;i++)
