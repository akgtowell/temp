...
{
    data = {};
    var Qry = "SELECT cm.customercode,CASE WHEN '" + sessionStorage.getItem("Language") + "' = 'en' THEN customername ELSE arbcustomername END AS 'customername',CASE WHEN '" + sessionStorage.getItem("Language") + "' = 'en' THEN customeraddress1 ELSE arbcustomeraddress1 END AS 'address',typecode,CAST(amount AS VARCHAR) amount,bankname,checkdate,checknumber,invoicenumber,comments,printstatus,(SELECT CASE cm.messagekey5 WHEN 0 THEN '' ELSE messageline1 || messageline2 || messageline3 || messageline4 END FROM customermessages WHERE messagekey = messagekey5) AS invheadermsg,IFNULL(excesspayment,'') AS excesspayment,cm.alternatecode,(select ifnull(sum(ci.invoicebalance),0) from customerinvoice ci where ci.customercode=arh.customercode ) as balancedue FROM arheader arh JOIN customermaster cm ON cm.customercode = arh.customercode JOIN cashcheckdetail ccd ON ccd.visitkey = arh.visitkey LEFT JOIN bankmaster bm ON bm.bankcode = ccd.bankcode WHERE arh.invoicenumber = " + invoicenumber;
    console.log(Qry);                
    if (platform == 'iPad') {
        Cordova.exec(function(result) 
        {
    ...
    result[0][6] = (d.getDate() + "/" + (d.getMonth() + 1) + "/" + d.getFullYear());
        ...
        window.plugins.DataBaseHelper.select(Qry, function(result) {
            ...
            {
                result = $.map(result.array, function(item, index) {
                        return [[item.customercode,item.customername, item.address,item.typecode,item.amount,item.bankname,item.checkdate,item.checknumber,item.invoicenumber,item.comments,item.printstatus,item.invheadermsg,item.excesspayment,item.alternatecode,item.balancedue]];
                });
                customercode = (result[0][0]).toString();
                customername = (result[0][1]).toString();
...
function PrintAgingAnalysis(customercode,name){
	data={};
	var Qry = "SELECT invoicenumber,strftime('%d-%m-%Y',transactiondate) transactiondate,ifnull(strftime('%d-%m-%Y',duedate),'NONE') duedate,invoicebalance as totalinvoiceamount ,remarks1,amountpaid,invoicebalance,ifnull(pdcbalance,0) pdcbalance,erpreferencenumber as erpnumber,salesmancode,ifnull(invoicebalance,0)-ifnull(pdcbalance,0) as invoicebalance FROM customerinvoice WHERE invoicebalance <> 0  AND voidflag IS NOT 1 AND customercode = " + customercode +" ORDER BY strftime('%Y-%m-%d',duedate),invoicenumber";
    console.log(Qry);                
    if (platform == 'iPad') {
        Cordova.exec(function(result) 
        {
        ...
        window.plugins.DataBaseHelper.select(Qry, function(result) {
            ...
            {
                result = $.map(result.array, function(item, index) {
                        return [[item.invoicenumber,item.transactiondate,item.duedate, item.totalinvoiceamount,item.remarks1,item.pdcbalance,item.invoicebalance]];
                });
            }
            else
                result = [];
...
function parseAgingAnalysis(customercode,name,result){
    ...
    data["HEADERS"] = ["Invoice#","Invoice Date","Due Date","Due Amount","Salesman","PDC","Balance Amount"];
    var totalamount = 0;   
    var totalpdc = 0;
    var totalbalance = 0;
    for(i=0;i<result.length;i++)
    {
        for(j=0;j<result[i].length;j++)
        {
            ...
            if(j==3)
                totalamount += eval(result[i][j]);
            if(j==5)
            	totalpdc += eval(result[i][j]);
            if(j==6)
            	totalbalance += eval(result[i][j]);
        }
    }
    if(isNaN(totalamount))
        totalamount = 0;
    ...
    if(isNaN(totalbalance))
    	totalbalance = 0;
    totalamount = eval(parseFloat(totalamount)).toFixed(decimalplace);
    totalpdc = eval(parseFloat(totalpdc)).toFixed(decimalplace);
    totalbalance = eval(parseFloat(totalbalance)).toFixed(decimalplace);
    data["TOTAL"] = {"Due Amount" : totalamount,"PDC": totalpdc,"Balance Amount": totalbalance};
    data["data"] = result;
	...
	{    
     ...
     else
     {
         ...
         //	------Start
          ...
          if(printval == 3) // dot matric printer
          {
          // alert("douut");
