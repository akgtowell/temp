...
function getinvoicedetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,transactionkey,CAST(itemcode AS VARCHAR) itemcode,salesqty,returnqty,damagedqty,freesampleqty,salesprice,returnprice,stdsalesprice,stdreturnprice,promoqty,salesitemexcisetax,salesitemgsttax,returnitemexcisetax,returnitemgsttax,damageditemexcisetax,damageditemgsttax,fgitemexcisetax,fgitemgsttax,promoitemexcisetax,promoitemgsttax,buybackexcisetax,buybackgsttax,coopid,batchdetailkey,salescaseprice,returncaseprice,stdsalescaseprice,stdreturncaseprice,goodreturnprice,goodreturncaseprice,stdgoodreturncaseprice,stdgoodreturnprice,expiryqty,currencycode,returnfreeqty,manualfreeqty,limitedfreeqty,rebaterentqty,fixedrentqty,pricechgindicator,discountamount,discountpercentage,promoamount,replacementqty,replacementprice,replacementcaseprice,promovalue,mdat,returnpromovalue,returnpromoamount,amount,diffround,roundsalesamount,amount,expiryitemgsttax,damagepromoamount from invoicedetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
...
function getinvoicerxddetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,transactionkey,itemtransactiontype,CAST(itemcode AS VARCHAR) itemcode,itemtranstypeseq,reasoncode,quantity,catchweightqty,weighted,instructioncode,currencycode,substr(expirydate, 7, 4) || '-' || substr(expirydate, 4, 2) || '-' || substr(expirydate, 1, 2) as expirydate,invoicenumber,batchnumber from invoicerxddetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and transactionkey in ("
								+ transactionkey + ")",
						function(result) {
...
function getorderrxddetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,transactionkey,itemtransactiontype,CAST(itemcode AS VARCHAR) itemcode,itemtranstypeseq,reasoncode,quantity,catchweightqty,weighted,instructioncode,substr(expirydate, 7, 4) || '-' || substr(expirydate, 4, 2) || '-' || substr(expirydate, 1, 2) as expirydate from orderrxddetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and transactionkey in ("
								+ transactionkey + ")",
						function(result) {
...
function getbatchexpirydetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,batchdetailkey,batchnumber,CAST(itemcode AS VARCHAR) itemcode,expirydate,quantity,transactiontypecode,visitkey from batchexpirydetail where issync=0 and istemp='false and routekey in ("
								+ routekey
								+ ") and visitkey in ("
								+ visitkey
								+ ")'", function(result) {
...
function getinventorytransactiondetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,detailkey,transactiontypecode,CAST(itemcode AS VARCHAR) itemcode,quantity,weighted,itemprice,batchdetailkey,itemcaseprice,currencycode,reasoncode,expirydate from inventorytransactiondetail where issync=0 and istemp='false' and routekey in ("
								+ routekey
								+ ") and detailkey in ("
								+ visitkey + ") ",
						function(result) {
...
function getposequipmentchangedetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,posaction,CAST(itemcode AS VARCHAR) itemcode,quantity,serialnumber,instructioncode from posequipmentchangedetail where issync=0 and routekey in ("
								+ routekey
								+ ") and visitkey in ("
								+ visitkey
								+ ")",
...
function getposmaster() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select CAST(itemcode AS VARCHAR) itemcode,alternatecode,itemdescription,arbitemdescription,itemvalue,inventorytype,created,cdat,modified,mdat,activestatus from posmaster where issync=0",
						function(result) {
							if (!$.isEmptyObject(result)) {
								for (i = 0; i < result.array.length; i++) {
...
function getcustomerinventorydetail() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select routekey,visitkey,CAST(itemcode AS VARCHAR) itemcode,weighted,qtyloc1case,catchweightqtyloc1,qtyloc1each,qtyloc2case,catchweightqtyloc2,qtyloc2each,qtyloc3case,catchweightqtyloc3,qtyloc3each,shelfstockcase,shelfstockcatchweightqty,shelfstockeach,oldestcode from customerinventorydetail where issync=0  and routekey in ("
								+ routekey
								+ ") and visitkey in ("
								+ visitkey
								+ ")",
...
function getcustomer_foc_balance() {
	if (platform == 'iPad') {
		...
		window.plugins.DataBaseHelper
				.select(
						"select customercode,CAST(itemcode AS VARCHAR) itemcode,originalqty,balanceqty,contractid,startdate from customer_foc_balance where issync=0",
						function(result) {
							if (!$.isEmptyObject(result)) {
								for (i = 0; i < result.array.length; i++) {
...
function UploadImages(fileName, filePath) {
	...
	ft.upload(filePath,
			"http://mpfuae.dyndns.org:8095/sfa/modernpoultry/api/image/upload", win,
			fail, options);
