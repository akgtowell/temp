...
function footerredict(url) {
	...
	if (url == '../settlement/managesettlement.html') {
		...
		footerurl = url;
		navigator.notification.confirm("Are You Sure To Cancel This Screen?",
				onConfirmfooter);
	}
...
function getSendData(routekey, transactionkey, itemcode, tableName, flag) {
	...
	// Required route key and transaction key as a parameter
		...
		tablequery = "select routekey,visitkey,transactionkey,itemtransactiontype,itemcode,itemtranstypeseq,reasoncode,quantity,catchweightqty,weighted,instructioncode,currencycode from invoicerxddetail where issync=0 and istemp='false' and routekey in ("
				+ routekey + ") and transactionkey in (" + transactionkey + ")";
	}
	...
	// Required detailkey = transaction key as a parameter
		...
		tablequery = "select routekey,detailkey,transactiontypecode,itemcode,quantity,weighted,itemprice,batchdetailkey,itemcaseprice,currencycode from inventorytransactiondetail where issync=0 and istemp='false' and routekey in ("
	...
	// Required itemcode = transaction key as a parameter
		...
		tablequery = "select visitkey,routekey,customercode,routecode,salesmancode,odometerreading,visitstartdate,visitstarttime,visitenddate,visitendtime,totaltransactions,addedcustomer,voidflag,scannerindicator,reasoncode,latitude,longitude,radius from customeroperationscontrol where issync=0 and routekey in ("
				+ routekey + ") and visitkey in (" + transactionkey + ")";
...
function uploadServerData() {
	...
	var dataarray = [ "invoicedetail", "invoiceheader", "invoicerxddetail",
			"promotiondetail", "customerinvoice", "salesorderdetail",
			"salesorderheader", "batchexpirydetail", "arheader", "ardetail",
			"cashcheckdetail", "inventorytransactionheader",
			"inventorytransactiondetail", "inventorysummarydetail",
			"nonservicedcustomer", "surveyauditdetail",
			"posequipmentchangedetail", "posmaster", "sigcapturedata",
			"customerinventorydetail", "customeroperationscontrol",
			"routesequencecustomerstatus", "routemaster", "routegoal" ];
		...
		Cordova.exec(function(result) {
			if (result.length > 0) {
				...
				DocumentNumber = DocPrefix.toString()
						+ (eval(result[0][0]) + 1).toString().lpad("0", 7);
...
function UpdateTotalInvoiceAmount(roundamount, status) {
	var QryUpdate = "Update invoiceheader set totalinvoiceamount=(ifnull(totalsalesamount,0)-ifnull(totalreturnamount,0)-ifnull(totalpromoamount,0)-ifnull(totaldamagedamount,0)+(select sum(diffround) from invoicedetail where routekey="
			+ sessionStorage.getItem("RouteKey")
			+ " AND visitkey="
...
function updateinvoiceround(amount, itemcode, diffround, flag) {
	var updateqry = "update invoicedetail set roundsalesamount=" + amount
