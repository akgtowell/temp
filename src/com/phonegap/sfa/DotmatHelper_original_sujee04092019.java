...
public class DotmatHelper_original_sujee04092019 extends Plugin {
	...
	/*
	 * int startln = 4;//4
	int startsalesln = 2; // sujee added for transactions
	int endln = 18;  // 12  // 24 
	int cnln = 8;  // 15
	    */
	int startln = 4;//4
	...
	void parseLoadSummaryResponse(final JSONObject object, final String... args) {
		...
		try {
			...
			hashValues.put("UPC", 8);
			hashValues.put("Open Qty", 15); //12
			hashValues.put("Load Qty", 15);//12
			hashValues.put("Adjust Qty", 15);//12
			hashValues.put("Net Qty", 20); //12 
			hashValues.put("VALUE", 0); //12
			hashValues.put("Description", 50);
		...
		try {
			...
			if (object.getString("displayupc").equals("1")) {
				...
				if (printoultlet != 0) {
					...
					hashValues.put("OUTLET CODE", 0);
					hashValues.put("DESCRIPTION", 48); //43 // 48
					hashValues.put("REBATE", 12);//0

				}
				hashValues.put("EXC TAX", 0);
            	hashValues.put("VAT(5%)", 0);
				if (printtax > 0) {
		              ...
		              if(excTot>0){
	                    ...
	                    if(vatTot>0){
	                    	hashValues.put("EXC TAX", 0);
	                    	hashValues.put("VAT(5%)", 11); //15
	                    	hashValues.put("AMOUNT", 17);//25
	                    }
	                    if(excTot>0&&vatTot>0){
	                    	hashValues.put("EXC TAX", 12);
	                    	hashValues.put("VAT(5%)", 11);
				...
				hashArabVales.put("REBATE", ArabicTEXT.DISCOUNT);
			...
			if (object.has("TOTEXC")) {
				...
				if(Double.parseDouble(object.getString("TOTEXC"))!=0){
					...
					if(companyTaxStng==1&&taxSetting!=1){
						printlines2(
								(getAccurateText("TOTAL EXC TAX", 20, 0) + getAccurateText(" : ", 3, 0)
										+ getAccurateText(object.getString("TOTEXC") + " AED", 12, 0)
										+ getAccurateText(" : ", 3, 0) + "*"
										+ getAccurateText(ArabicTEXT.TotalExcTax, 20, 2) + "!"),
								1, object, 1, args[0], 1, 1);
					}
			...
			if (object.has("TOTVAT")) {
				...
				if(Double.parseDouble(object.getString("TOTVAT"))!=0){
					...
					if(companyTaxStng==1&&taxSetting!=1){
						printlines2(
								(getAccurateText("TOTAL VAT TAX", 20, 0) + getAccurateText(" : ", 3, 0)
										+ getAccurateText(object.getString("TOTVAT") + " AED", 12, 0)
										+ getAccurateText(" : ", 3, 0) + "*"
										+ getAccurateText(ArabicTEXT.TotalVatTax, 20, 2) + "!"),
								1, object, 1, args[0], 1, 1);
					}
		...
		try {
			...
			if (object.has("ORDER DISCOUNT") && object.getString("ORDER DISCOUNT").toString().length() > 0) {
				...
				if (invoice != 0) {
					...
					printlines2(
							(getAccurateText("ORDER DISCOUNT", 20, 0) + getAccurateText(" : ", 3, 0)
									+ getAccurateText(object.getString("ORDER DISCOUNT") + " AED", 12, 0)
									+ getAccurateText(" : ", 3, 0) + "*"
									+ getAccurateText(ArabicTEXT.OrderDiscount, 15, 2) + "!"),
							1, object, 1, args[0], 1, 1);
				}
			...
			printlines2(
					(getAccurateText("NET AMOUNT", 20, 0) + getAccurateText(" : ", 3, 0)
							+ getAccurateText(object.getString("NET AMOUNT") + " AED", 12, 0)
	...
	private void printlines1(String data, int ln, JSONObject object, int sts, String adr, int tp)
		...
		if (sts == 2 && count != 0) {
			...
			lnno = pln - count;
		//	lnno = lnno + endln; // org line 30/01/2019
			// sujee added 18/06/2019  for MPF oman and Sharjah
		...
		//	lnno = lnno - endln;
			...
			outStream.write(CarriageReturn); // sujee commented temp 
			// outStream.write(NewLine);
			// outStream.write(NewLine);
			// outStream.write(NewLine);
			count = 0;
	...
	private void printlines2(String data, int ln, JSONObject object, int sts, String adr, int tran, int tp)
		...
		count += ln;
		linecnt++;
		int pln = 48; // 48 // sujee chged for 6824 printer 13/06/2018 //28 //24  //25 //29 ------31
	...
	private void headerTaxprint(JSONObject object, int type, String... args) throws JSONException {
		try {
			...
			/* sujee commented as no need to print header for MPF 05/06/2018
			 * outStream.write(BoldOn);
			...
			outStream.write(NewLine);
              ...
              if (object.has("companytaxregistrationnumber")){
				...
				printArabic(getAccurateText("Company Tax Reg. No."+object.getString("companytaxregistrationnumber"), 40, 0));
				outStream.write(NewLine);
				outStream.write(NewLine);
			}
			*/
			...
			if (type == 1) {
				...
				if (!object.getString("LANG").equals("en")) {
					...
					if (object.getString("invoicepaymentterms").contains("2")) {
						...
						/*printArabic(getAccurateText(
								"Invoice No. " + object.getString("invoicenumber"), 40, 1));*/
						printArabic(getAccurateText(
								object.getString("INVTYPE") +"Invoice No. " + object.getString("invoicenumber"), 40, 1));


					} else {
						outStream.write((getAccurateText(object.getString("INVOICETYPE"), 40, 1)).getBytes());
						outStream.write(NewLine);
						outStream.write((getAccurateText("TRN NO: 100342783600003", 80, 1)).getBytes());
						outStream.write(NewLine);
				...
				/*	outStream.write((getAccurateText(
					...
					outStream.write((getAccurateText(
							object.getString("INVTYPE") + "Invoice No. " + object.getString("invoicenumber"), 40, 1)).getBytes());
				...
				try {
					...
					outStream.write((getAccurateText("CUSTOMER : " +object.getString("CUSTOMERID"), 19, 1)).getBytes());
					outStream.write(NewLine);
			...
			/*	outStream.write(("ADDRESS: " + object.getString("ADDRESS")).getBytes());*/
				...
				if (object.has("taxregistrationnumber")){
					...
					printArabic(getAccurateText("TRN No : " + object.getString("taxregistrationnumber"), 40, 0) + getAccurateText(
							"Trip ID : " + object.getString("TourID"), 40, 2));
					outStream.write(NewLine);
				...
				printArabic(getAccurateText(" ", 40, 0) +getAccurateText("SUPERVISOR : " + object.getString("supervisorname") + "-" + object.getString("supervisorno"), 40, 2));
				outStream.write(NewLine);
