...
public class DotmatHelper extends Plugin {
	...
	void parseLoadSummary2Response(final JSONObject object, final String... args) {
		...
		try {
			...
			/*
			 * Paireddevicename = devicename.substring(0,4); Log.e("Paireddevicename", "" +
			 * Paireddevicename); if(Paireddevicename.equals("6820") ||
			 * Paireddevicename.equals("6822")) { line(4); } else { line(8); }
			 */
	...
	void parseLoadSummaryResponse(final JSONObject object, final String... args) {
		...
		try {
			 ...
			 * getAccurateText(object.getString("LoadValue"), 12, 2)), 1, object, 1,
			 * args[0], 2);
			 */
			// printlines1(
	...
	void parseSockMoveResponse(final JSONObject object, final String... args) {
		...
		try {
			...
			for (int i = 0; i < headers.length(); i++) {
				...
				/*
				 * strheader = strheader + getAccurateText( (headers.getString(i).indexOf(" ")
				 * == -1) ? headers.getString(i) : headers.getString(i),
				 * hashValues.get(headers.getString(i).toString()) + MAXLEngth,
				 * hashPositions.get(headers.getString(i).toString()));
				 */
	...
	void parseFreeSummaryResponse(final JSONObject object, final String... args) {
		...
		try {
			...
			for (int i = 0; i < headers.length(); i++) {
				...
				strheader = strheader + getAccurateText(
						(headers.getString(i).indexOf(" ") == -1) ? headers.getString(i) : headers.getString(i),
						hashValues.get(headers.getString(i).toString()) + MAXLEngth,
						hashPositions.get(headers.getString(i).toString()));
	...
	void printVanStockReport(final JSONObject object, final String... args) {
		...
		try {
			...
			/*
			 * outStream.write(CompressOn); printlines1(printSepratorcomp(), 1, object, 1,
			 * args[0], 4); // outStream.write(CompressOn); printlines1(strTotal, 1, object,
			 * 1, args[0], 4); //outStream.write(CompressOff);
			 * //printlines1(printSepratorcomp(), 1, object, 1, args[0], 4);
			 * outStream.write(CompressOff); outStream.write(NewLine);
			 */
			...
			outStream.write(NewLine);
			outStream.write(NewLine);
			printlines1(getAccurateText("SALESMAN_______________*" + ArabicTEXT.Salesman + "!", 80, 1), 2, object, 2,
					args[0], 1);
			outStream.write(NewLine);
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			for (int i = 0; i < jData.length(); i++) {
				...
				for (int j = 0; j < headers.length(); j++) {
					...
					if (j != 0 && j != 5) {
						/*
						 * if (j == 2 && printoultlet == 0) { Log.e("isOutlet", "true"); isoutlet =
						 * true; }
						 */
						...
						if (!isoutlet) {
							...
							if (jTotal.has(headers.getString(j))) {
								if (j == 11) {
									strTotal = strTotal
											+ getAccurateText(jTotal.getString(headers.getString(12).toString()),
													hashValues.get(headers.getString(j).toString()) + MAXLEngth,
													hashPositions.get(headers.getString(j).toString()));
								} else if (j == 12) {
									strTotal = strTotal
											+ getAccurateText(jTotal.getString(headers.getString(11).toString()),
													hashValues.get(headers.getString(j).toString()) + MAXLEngth,
													hashPositions.get(headers.getString(j).toString()));
								} else {
									strTotal = strTotal
				...
				for (int l = 0; l < jInnerData.length(); l++) {// outStream.write(CompressOn);
					...
					for (int m = 0; m < jArr.length(); m++) {
						if (m != 0 && m != 5) {
							isoutletdata = false;
							/*
							 * if (m == 2 && printoultlet == 0) { isoutletdata = true; }
							 */
			...
			if (object.has("TOTSALES")) {
				if (Double.parseDouble(object.getString("TOTSALES")) != 0) {
					outStream.write(BoldOn);
					printlines2((getAccurateText("SALES AMOUNT", 20, 0) + getAccurateText(" : ", 3, 0)
			...
			if (object.has("TOTRETURNAMT")) {
				if (Double.parseDouble(object.getString("TOTRETURNAMT")) != 0) {
					outStream.write(BoldOn);
					printlines2((getAccurateText("RETURN AMOUNT", 20, 0) + getAccurateText(" : ", 3, 0)
							+ getAccurateText("-" + object.getString("TOTRETURNAMT") + " OMR", 12, 0)
							+ getAccurateText(" : ", 3, 0) + "*" + getAccurateText(ArabicTEXT.RetAmt, 15, 2) + "!"), 1,
							object, 1, args[0], 1, 1);
					outStream.write(BoldOff);
				}
			}
			...
			outStream.write(BoldOn);
			printlines2((getAccurateText("AMOUNT BEFORE VAT", 20, 0) + getAccurateText(" : ", 3, 0)
					+ getAccurateText(object.getString("NET SALES") + " OMR", 12, 0) + getAccurateText(" : ", 3, 0)
					+ "*" + getAccurateText(ArabicTEXT.AmtBeforeVat, 15, 2) + "!"), 1, object, 1, args[0], 1, 1);
			outStream.write(BoldOff);
			...
			if (object.has("INVOICE DISCOUNT") && object.getString("INVOICE DISCOUNT").toString().length() > 0) {
				...
				if (invoice != 0) {
					...
					/*
					 * printlines2( (getAccurateText(object.getString("REBATE")+" %"+" REBATE ", 20,
					 * 0) + getAccurateText(" : ", 3, 0) +
					 * getAccurateText(object.getString("INVOICE DISCOUNT") + " OMR", 12, 0) +
					 * getAccurateText(" : ", 3, 0) + "*" +
					 * getAccurateText(ArabicTEXT.InvoiceDiscount, 15, 2) + "!"), 1, object, 1,
					 * args[0], 1, 1);
					 */
			...
			outStream.write(BoldOn);
			printlines2((getAccurateText("AMOUNT AFTER VAT", 20, 0) + getAccurateText(" : ", 3, 0)
					+ getAccurateText(object.getString("SUB TOTAL") + " OMR", 12, 0) + getAccurateText(" : ", 3, 0)
					+ "*" + getAccurateText(ArabicTEXT.AmtAfterVat, 15, 2) + "!"), 1, object, 1, args[0], 1, 1);
			outStream.write(BoldOff);
			...
			printlines2(getAccurateText("CUSTOMER_________________*" + ArabicTEXT.Customer
					+ "!   SALESMAN_______________*" + ArabicTEXT.Salesman + "!", 90, 1), 2, object, 1, args[0], 1, 1);
			outStream.write(NewLine);
			String copyStatus = "";
			if (object.getString("printstatus").equals("DUPLICATE COPY")) {
			...
			for (int i = 0; i < jData.length(); i++) {
				...
				for (int j = 0; j < headers.length(); j++) {
					...
					if (!isoutlet) {
						...
						strheader = strheader
								+ getAccurateText(HeaderVal, hashValues.get(HeaderVal.toString()) + MAXLEngth,
										hashPositions.get(HeaderVal.toString()));
	...
	void printSalesReportNew(JSONObject object, String... args) {
		...
		try {
			...
			for (int i = 0; i < jData.length(); i++) {
				...
				for (int j = 0; j < headers.length(); j++) {
					...
					if (!isoutlet) {
						...
						/*
						 * strHeaderBottom = strHeaderBottom + getAccurateText( (HeaderVal.indexOf(" ")
						 * == -1) ? "" : HeaderVal.substring(HeaderVal.indexOf(" "), HeaderVal.length())
						 * .trim(), hashValues.get(HeaderVal.toString()) + MAXLEngth,
						 * hashPositions.get(HeaderVal.toString()));
						 */
						if (jTotal.has(headers.getString(j))) {
							strTotal = strTotal + getAccurateText(jTotal.getString(headers.getString(j).toString()),
							...
							strTotal = strTotal + getAccurateText(
									headers.getString(j).equals("DESCRIPTION") ? getAccurateText(" ", 50, 0) + "TOTAL"
											: "",
									hashValues.get(headers.getString(j)) + MAXLEngth, 1);
						}
					}
				...
				if (jInnerData.length() > 0) {
					...
					printlines2(strheader, 1, object, 1, args[0], 1, 1);
					/*
					 * if (strHeaderBottom.length() > 0) { printlines2(strHeaderBottom, 1, object,
					 * 1, args[0], 1, 1); }
					 */
					// }
				...
				for (int l = 0; l < jInnerData.length(); l++) {
					 ...
					 * for (int m = 0; m < jArr.length(); m++) {
					 * 
					 * isoutletdata = false; if (m == 2 && printoultlet == 0) { isoutletdata = true;
					 * }
					 * 
					 * if (!isoutletdata) {
					 * 
					 * String itemDescrion = jArr.getString(m); if (m == 0) { itemDescrion = (l + 1)
					 * + "";
					 * 
					 * } else if (m == 13) { if(object.getString("printbarcode").equals("1")){
					 * itemDescrion = "               " + jArr.getString(m) + ""; }else{
					 * itemDescrion = "              *" + jArr.getString(m) + "!"; }
					 * 
					 * 
					 * }
					 * 
					 * if(m!=13) { strData = strData + getAccurateText(itemDescrion, m == 12 ? 65 :
					 * hashValues.get(headers.getString(m).toString()) + MAXLEngth,
					 * hashPositions.get(m == 12 ? "DESCRIPTION" :
			...
			printlines2((getAccurateText("NET DUE THIS AMOUNT", 20, 0) + getAccurateText(" : ", 3, 0)
					+ getAccurateText(object.getString("SUB TOTAL"), 8, 0)
					+ getAccurateText(object.getString("currname"), 3, 0) + getAccurateText(" : ", 3, 0) + "*"
			...
			if (object.has("INVOICE DISCOUNT") && object.getString("INVOICE DISCOUNT").toString().length() > 0) {
				...
				if (invoice != 0) {
					...
					printlines2(
							(getAccurateText("INVOICE DISCOUNT", 20, 0) + getAccurateText(" : ", 3, 0)
									+ getAccurateText(object.getString("INVOICE DISCOUNT"), 8, 0)
									+ getAccurateText(object.getString("currname"), 3, 0) + getAccurateText(" : ", 3, 0)
									+ "*" + getAccurateText(ArabicTEXT.InvoiceDiscount, 15, 2) + "!"),
							1, object, 1, args[0], 1, 1);
				}
			...
			printlines2((getAccurateText("NET SALES", 20, 0) + getAccurateText(" : ", 3, 0)
					+ getAccurateText(object.getString("NET SALES"), 8, 0)
					+ getAccurateText(object.getString("currname"), 3, 0) + getAccurateText(" : ", 3, 0) + "*"
			...
			printlines2((getAccurateText("GOOD RETURNS", 20, 0) + getAccurateText(" : ", 3, 0)
					+ getAccurateText(object.getString("TOTGOOD"), 8, 0)
					+ getAccurateText(object.getString("currname"), 3, 0) + getAccurateText(" : ", 3, 0) + "*"
...
//	outStream.write(BoldOff);
			...
			printlines2((getAccurateText("DAMAGE RETURNS", 20, 0) + getAccurateText(" : ", 3, 0)
					+ getAccurateText(object.getString("TOTBAD"), 8, 0)
					+ getAccurateText(object.getString("currname"), 3, 0) + getAccurateText(" : ", 3, 0) + "*"
...
//outStream.write(BoldOff);
			...
			if (object.getString("invoicepaymentterms").contains("2")) {
				...
				outStream.write(BoldOn);
				printlines2(
						(getAccurateText(" ", 20, 0) + getAccurateText("GC CHARGED", 20, 0)
								+ getAccurateText(" : ", 3, 0) + getAccurateText(object.getString("SUB TOTAL"), 8, 0)
								+ getAccurateText(object.getString("currname"), 3, 0) + getAccurateText(" : ", 3, 0)
								+ "*" + getAccurateText(ArabicTEXT.NetSales, 15, 2) + "!"),
						1, object, 1, args[0], 1, 1);
				outStream.write(BoldOff);
			...
			if (object.has("TCALLOWED") && object.getString("TCALLOWED").toString().trim().length() > 0
				...
				printlines2(
						(getAccurateText("TC CHARGED", 20, 0) + getAccurateText(" : ", 3, 0)
								+ getAccurateText(object.getString("TCCHARGED"), 8, 0)
								+ getAccurateText(object.getString("currname"), 3, 0) + getAccurateText(" : ", 3, 0)
								+ "*" + getAccurateText(ArabicTEXT.TCcharged, 15, 2) + "!"),
						1, object, 1, args[0], 1, 1);
			...
			if (object.has("PaymentType") && Integer.parseInt(object.getString("PaymentType")) < 2) {
				...
				/*
				 * outStream.write(BoldOn); if (!object.getString("LANG").equals("en")) {
				 * 
				 * printlines2(getAccurateText("*" + ArabicTEXT.PaymentDetails + "!", 80, 1), 2,
				 * object, 1, args[0], 1, 1); } else {
				 * printlines2(getAccurateText("PAYMENT DETAILS", 80, 1), 2, object, 1, args[0],
				 * 1, 1); }
				 * 
				 * outStream.write(BoldOff);
				 */
	...
	/*
	 * void printSalesReport(JSONObject object, String... args) { StringBuffer s1 =
	 * new StringBuffer(); int printoultlet = 0;
	 * 
	 * try {
	 * 
	 * 
	 ...
	 * String pcs = object.has("pcs") ? object.getString("pcs") : "Unit"; String qty
	 * = object.has("qty") ? object.getString("qty") : "QTY CAS/PCS"; String barcode
	 * = object.has("barcode") ? object.getString("barcode") : "Barcode"; String
	 * printLanguage=object.has("printlanguageflag")?object.getString(
	 * "printlanguageflag"):"2"; int CaseEnabled = object.has("CaseEnabled") ?
	 * Integer.parseInt(object.getString("CaseEnabled")) : 1; if
	 * (object.getString("printoutletitemcode").length() > 0) { printoultlet =
	 * Integer.parseInt(object.getString("printoutletitemcode")); } else {
	 * printoultlet = 0; } int printitemcode =
	 * Integer.valueOf(object.getString("printitemcode")); int printtax =
	 * Integer.parseInt(object.getString("printtax")); int totaldiscount =
	 * object.has("discountprint") ?
	 * Integer.parseInt(object.getString("discountprint")) : 0; double
	 * totaldiscountval = object.has("TOTAL LINE DISCOUNT") ?
	 * Double.parseDouble(object.getString("TOTAL LINE DISCOUNT")) : 0; String
	 * totalpcs = object.has("totalpcs") ? object.getString("totalpcs") : "0"; if
	 * (object.getString("displayupc").equals("1")) { hashValues = new
	 * HashMap<String, Integer>(); hashValues.put("SL#", 3); hashValues.put("ITEM#",
	 * 11); if (printoultlet == 1) { hashValues.put("OUTLET CODE", 15);
	 * 
	 * if (printitemcode == 0) { hashValues.put("ITEM#", 0);
	 * if(printLanguage.equals("2")){ hashValues.put("DESCRIPTION", 33);
	 * hashValues.put("ARBDESCRIPTION", 29); }else if(printLanguage.equals("1")){
	 * hashValues.put("DESCRIPTION", 0); hashValues.put("ARBDESCRIPTION", 62);
	 * }else{ hashValues.put("DESCRIPTION", 62); hashValues.put("ARBDESCRIPTION",
	 * 0); }
	 * 
	 * } else { hashValues.put("ITEM#", 11); if(printLanguage.equals("2")){
	 * hashValues.put("DESCRIPTION", 22); hashValues.put("ARBDESCRIPTION", 29);
	 * }else if(printLanguage.equals("1")){ hashValues.put("DESCRIPTION", 0);
	 * hashValues.put("ARBDESCRIPTION", 51); }else{ hashValues.put("DESCRIPTION",
	 * 51); hashValues.put("ARBDESCRIPTION", 0); }
	 * 
	 * } } else { hashValues.put("OUTLET CODE", 0); if (printitemcode == 0) {
	 * hashValues.put("ITEM#", 0); if(printLanguage.equals("2")){
	 * hashValues.put("DESCRIPTION", 48); hashValues.put("ARBDESCRIPTION", 29);
	 * }else if(printLanguage.equals("1")){ hashValues.put("DESCRIPTION", 0);
	 * hashValues.put("ARBDESCRIPTION", 77); }else{ hashValues.put("DESCRIPTION",
	 * 77); hashValues.put("ARBDESCRIPTION", 0); }
	 * 
	 * } else { hashValues.put("ITEM#", 11); if(printLanguage.equals("2")){
	 * hashValues.put("DESCRIPTION", 37); hashValues.put("ARBDESCRIPTION", 29);
	 * }else if(printLanguage.equals("1")){ hashValues.put("DESCRIPTION", 0);
	 * hashValues.put("ARBDESCRIPTION", 66); }else{ hashValues.put("DESCRIPTION",
	 * 66); hashValues.put("ARBDESCRIPTION", 0); }
	 * 
	 * } }
	 * 
	 * hashValues.put(barcode, 29); hashValues.put("UOM", 5); hashValues.put(qty,
	 * 8); hashValues.put("" + pcs, 0); hashValues.put("GROSS AMOUNT", 0);
	 * hashValues.put("EXCISE TAX", 0); hashValues.put("NET PRICE", 0);
	 * hashValues.put("VAT AMOUNT", 0); if (totaldiscount == 0 && totaldiscountval >
	 * 0) { hashValues.put("DISCOUNT", 17); if (totalpcs.equals("1")) {
	 * hashValues.put("" + pcs, 4); if(CaseEnabled==1){ hashValues.put(cases +
	 * " PRICE", 6); hashValues.put(pcs + " PRICE", 6); }else{ hashValues.put(cases
	 * + " PRICE", 0); hashValues.put(pcs + " PRICE", 12); }
	 * 
	 * 
	 * } else { hashValues.put("" + pcs, 0); if(CaseEnabled==1){
	 * hashValues.put(cases + " PRICE", 8); hashValues.put(pcs + " PRICE", 8);
	 * }else{ hashValues.put(cases + " PRICE", 0); hashValues.put(pcs + " PRICE",
	 * 16); }
	 * 
	 * } } else { hashValues.put("DISCOUNT", 0);
	 * 
	 * if (totalpcs.equals("1")) { hashValues.put("" + pcs, 4); if(CaseEnabled==1){
	 * hashValues.put(cases + " PRICE", 15); hashValues.put(pcs + " PRICE", 14);
	 * }else{ hashValues.put(cases + " PRICE", 0); hashValues.put(pcs + " PRICE",
	 * 29); }
	 * 
	 * } else { hashValues.put("" + pcs, 0); if(CaseEnabled==1){
	 * hashValues.put(cases + " PRICE", 16); hashValues.put(pcs + " PRICE", 17);
	 * }else{ hashValues.put(cases + " PRICE", 0); hashValues.put(pcs + " PRICE",
	 * 33); }
	 * 
	 * } }
	 ...
	 * hashPositions.put("ITEM#", 0); hashPositions.put("OUTLET CODE", 0);
	 * hashPositions.put("DESCRIPTION", 0); hashPositions.put("ARBDESCRIPTION", 2);
	 * hashPositions.put("UOM", 1); hashPositions.put("" + pcs, 1);
	 * hashPositions.put(qty, 1); hashPositions.put(cases + " PRICE", 2);
	 * hashPositions.put(pcs + " PRICE", 2); hashPositions.put(barcode, 2);
	 * hashPositions.put("GROSS AMOUNT", 2); hashPositions.put("DISCOUNT", 2);
	 * hashPositions.put("EXCISE TAX", 2); hashPositions.put("VAT AMOUNT", 2);
	 * hashPositions.put("NET PRICE", 2); hashPositions.put("AMOUNT", 2);
	 * 
	 * } else { hashValues = new HashMap<String, Integer>(); hashValues.put("SL#",
	 * 4); hashValues.put("ITEM#", 8); hashValues.put("OUTLET CODE", 8);
	 * hashValues.put("DESCRIPTION", 36); hashValues.put("QTY CAS/PCS", 3);
	 * hashValues.put("QTY OUT/PCS", 3); if(CaseEnabled==1){
	 * hashValues.put("CASE PRICE", 7); hashValues.put("UNIT PRICE", 7); }else{
	 * hashValues.put("CASE PRICE", 0); hashValues.put("UNIT PRICE", 14); }
	 * 
	 * hashValues.put("DISCOUNT", 0); hashValues.put("AMOUNT", 8);
	 * hashValues.put("OUTER PRICE", 7); hashValues.put("PCS PRICE", 7);
	 * hashPositions = new HashMap<String, Integer>(); hashPositions.put("SL#", 0);
	 * hashPositions.put("ITEM#", 0); hashPositions.put("OUTLET CODE", 0); //
	 * hashPositions.put("DESCRIPTION", 0); hashPositions.put("QTY CAS/PCS", 2);
	 * hashPositions.put("CASE PRICE", 2); hashPositions.put("UNIT PRICE", 2);
	 * hashPositions.put("DISCOUNT", 2); hashPositions.put("AMOUNT", 2);
	 * hashPositions.put("OUTER PRICE", 2); hashPositions.put("PCS PRICE", 2); } //
	 * ---------Start
	 * 
	 * // ----------End
	 * 
	 * if (!object.getString("invoiceformat").equals("1")) { // header is //
	 * printing // instead line(startln); } headerprint(object, 1);
	 * 
	 * JSONArray jData = object.getJSONArray("data"); for (int i = 0; i <
	 * jData.length(); i++) {
	 * 
	 * JSONObject mainJson = jData.getJSONObject(i); JSONArray jInnerData =
	 * mainJson.getJSONArray("DATA"); JSONArray headers =
	 * mainJson.getJSONArray("HEADERS"); JSONObject jTotal =
	 * mainJson.getJSONObject("TOTAL"); if (jInnerData.length() > 0) {
	 * 
	 * String header = mainJson.getString("TITLE").trim(); String HeadTitle = ""; if
	 * (header.equals("sales")) { if
	 * (object.getString("printlanguageflag").equals("2")) { HeadTitle = "SALES "; }
	 * else {
	 * 
	 * HeadTitle = "SALES  *" + ArabicTEXT.Sales + "!"; }
	 * 
	 * } else if (header.equals("free")) { if
	 * (!object.getString("printlanguageflag").equals("2")) {
	 * 
	 * HeadTitle = "TRADE DEAL  *" + ArabicTEXT.tradeDeal + "!"; } else { HeadTitle
	 * = "TRADE DEAL"; }
	 * 
	 * } else if (header.equals("bad")) {
	 * 
	 ...
	 * 
	 * HeadTitle = "BAD RETURN  *" + ArabicTEXT.BadReturn + "!"; } else { HeadTitle
	 * = "BAD RETURN"; }
	 * 
	 * } else if (header.equals("good")) { if
	 * (!object.getString("printlanguageflag").equals("2")) {
	 * 
	 * HeadTitle = "GOOD RETURN  *" + ArabicTEXT.GoodReturn + "!"; } else {
	 * HeadTitle = "GOOD RETURN"; }
	 * 
	 * } else if (header.equals("promofree")) { if
	 * (!object.getString("printlanguageflag").equals("2")) {
	 * 
	 * HeadTitle = "PROMOTION FREE  *" + ArabicTEXT.PromotionFree + "!"; } else {
	 * HeadTitle = "PROMOTION FREE"; }
	 * 
	 * } else if (header.equals("buyback")) { if
	 * (!object.getString("printlanguageflag").equals("2")) { HeadTitle =
	 * "BUYBACK FREE  *" + ArabicTEXT.BuybackFree + "!"; } else { HeadTitle =
	 * "BUYBACK FREE"; }
	 * 
	 * } outStream.write(BoldOn); // outStream.write(NewLine);
	 * outStream.write("       ".getBytes()); outStream.write(UnderlineOn);
	 * printlines2(HeadTitle, 1, object, 1, args[0], 1, 1); //
	 * outStream.write(NewLine); outStream.write(UnderlineOff);
	 * outStream.write(BoldOff);
	 * 
	 * } int MAXLEngth = 137;
	 * 
	 ...
	 * String HeaderVal = "";
	 * 
	 * HeaderVal = ArabicTEXT.getHeaderVal(headers.getString(j)); if (j == 4 &&
	 * object.getString("printbarcode").equals("1")) { HeaderVal = barcode; }
	 * strheader = strheader + getAccurateText( (j == 4 &&
	 * object.getString("printbarcode").equals("0")) ? "DESCRIPTION" :
	 * (HeaderVal.indexOf(" ") == -1) ? HeaderVal : HeaderVal.substring(0,
	 * HeaderVal.indexOf(" ")), hashValues.get(HeaderVal.toString()) + MAXLEngth,
	 * hashPositions.get(HeaderVal.toString()));
	 * 
	 * strHeaderBottom = strHeaderBottom + getAccurateText( (j == 4 &&
	 * object.getString("printbarcode").equals("0")) ? "" : (HeaderVal.indexOf(" ")
	 * == -1) ? "" : HeaderVal.substring(HeaderVal.indexOf(" "), HeaderVal.length())
	 * .trim(), hashValues.get(HeaderVal.toString()) + MAXLEngth,
	 * hashPositions.get(HeaderVal.toString()));
	 * 
	 ...
	 * hashValues.get(headers.getString(j)) + MAXLEngth, 1); }
	 * 
	 * } if (jInnerData.length() > 0) { outStream.write(CompressOn);
	 * 
	 * 
	 * printlines2(strheader, 1, object, 1, args[0], 1, 1); if
	 * (strHeaderBottom.length() > 0) { printlines2(strHeaderBottom, 1, object, 1,
	 * args[0], 1, 1); } // }
	 * 
	 * outStream.write(CompressOff);
	 * 
	 * outStream.write(CompressOn); printlines2(printSepratorcomp(), 1, object, 1,
	 * args[0], 1, 1); outStream.write(CompressOff); }
	 * 
	 * for (int l = 0; l < jInnerData.length(); l++) { JSONArray jArr =
	 * jInnerData.getJSONArray(l); String strData = "";
	 * 
	 * for (int m = 0; m < jArr.length(); m++) {
	 * 
	 * String itemDescrion = jArr.getString(m); if (m == 0) { itemDescrion = (l + 1)
	 * + "";
	 * 
	 * } else if (m == 4) { itemDescrion = ""; if
	 * (object.getString("printbarcode").equals("1")) { itemDescrion =
	 * jArr.getString(m); } else {
	 * 
	 * try { Arabic6822Length arabic6822Length=new Arabic6822Length(); itemDescrion
	 * = "*" + arabic6822Length.ConvertLength(jArr.getString(m), false) + "!"; }
	 * catch (Exception e) { e.printStackTrace(); } }
	 * 
	 * }
	 * 
	 * strData = strData + getAccurateText(itemDescrion, m == 3 ?
	 * hashValues.get(headers.getString(m).toString()) + MAXLEngth :
	 * hashValues.get(headers.getString(m).toString()) + MAXLEngth,
	 * hashPositions.get(headers.getString(m).toString()));
	 * 
	 * }
	 * 
	 * // lp.writeLine(strData); outStream.write(CompressOn); //isCompressModeOn =
	 * true; if (!object.getString("printlanguageflag").equals("2")) { // count++; }
	 * // printlines2(strData.trim(), 1, object, 1, args[0], 1, 1);
	 * 
	 * outStream.write(CompressOff); //isCompressModeOn = false; } if
	 * (jInnerData.length() > 0) { outStream.write(CompressOn); //isCompressModeOn =
	 * true; printlines2(printSepratorcomp(), 1, object, 1, args[0], 1, 1);
	 * printlines2(strTotal, 1, object, 1, args[0], 1, 1);
	 * outStream.write(CompressOff); //isCompressModeOn = false;
	 * 
	 * }
	 * 
	 * } outStream.write(NewLine);
	 * 
	 * if (object.has("TOTAL SALES AMOUNT")&&object.has("TOTAL PROMO FREE AMOUNT")&&
	 * object.has("TOTAL FREE AMOUNT")) {
	 * 
	 * double totalAmnt =
	 * Double.parseDouble(object.getString("TOTAL SALES AMOUNT"))+
	 ...
	 * if (!object.getString("printlanguageflag").equals("2")) {
	 * 
	 * printlines2( (getAccurateText("TOTAL SALES AMOUNT", 20, 0) +
	 * getAccurateText(" : ", 3, 0) + getAccurateText(strAmnt, 12, 0) +
	 * getAccurateText(" : ", 3, 0) + "*" +
	 * getAccurateText(ArabicTEXT.TotalSalesAmount, 15, 2) + "!"), 1, object, 1,
	 * args[0], 1, 1); } else { printlines2( (getAccurateText("TOTAL SALES AMOUNT",
	 * 20, 0) + getAccurateText(" : ", 3, 0) + getAccurateText(strAmnt, 12, 0) +
	 * getAccurateText(" : ", 3, 0) + "*" +
	 * getAccurateText(ArabicTEXT.TotalSalesAmount, 15, 2) + "!"), 1, object, 1,
	 * args[0], 1, 1); } }else if
	 * (object.has("TOTAL SALES AMOUNT")&&object.has("TOTAL PROMO FREE AMOUNT")) {
	 * 
	 * double totalAmnt =
	 * Double.parseDouble(object.getString("TOTAL SALES AMOUNT"))+
	 * Double.parseDouble(object.getString("TOTAL PROMO FREE AMOUNT"));
	 ...
	 * getAccurateText(ArabicTEXT.TotalSalesAmount, 15, 2) + "!"), 1, object, 1,
	 * args[0], 1, 1); } else { printlines2( (getAccurateText("TOTAL SALES AMOUNT",
	 * 65, 2) + getAccurateText(" : ", 3, 0) + getAccurateText(strAmnt, 12, 2)), 1,
	 * object, 1, args[0], 1, 1); } }else if (object.has("TOTAL SALES AMOUNT")) { if
	 * (!object.getString("printlanguageflag").equals("2")) {
	 * 
	 * printlines2( (getAccurateText("TOTAL SALES AMOUNT", 20, 0) +
	 * getAccurateText(" : ", 3, 0) +
	 ...
	 * getAccurateText(ArabicTEXT.TotalSalesAmount, 15, 2) + "!"), 1, object, 1,
	 * args[0], 1, 1); } else { printlines2( (getAccurateText("TOTAL SALES AMOUNT",
	 * 65, 2) + getAccurateText(" : ", 3, 0) +
	 * getAccurateText(object.getString("TOTAL SALES AMOUNT"), 12, 2)), 1, object,
	 * 1, args[0], 1, 1); } }
	 * 
	 * 
	 * 
	 ...
	 * 
	 * 
	 * printlines2( (getAccurateText("TOTAL GOOD RETURN", 20, 0) +
	 * getAccurateText(" : ", 3, 0) + getAccurateText(strAmnt, 12, 0) +
	 * getAccurateText(" : ", 3, 0) + "*" +
	 * getAccurateText(ArabicTEXT.TotalGoodReturn, 20, 2) + "!"), 1, object, 1,
	 * args[0], 1, 1);
	 * 
	 * }else if (object.has("TOTAL GOOD RETURN")) { printlines2(
	 ...
	 * 
	 * 
	 * if (object.has("TOTAL BAD RETURN")) { printlines2(
	 * (getAccurateText("TOTAL BAD RETURN", 20, 0) + getAccurateText(" : ", 3, 0) +
	 * getAccurateText(object.getString("TOTAL BAD RETURN"), 12, 0) +
	 * getAccurateText(" : ", 3, 0) + "*" +
	 * getAccurateText(ArabicTEXT.TotalBadReturn, 20, 2) + "!"), 1, object, 1,
	 * args[0], 1, 1);
	 * 
	 * } if (object.has("INVOICE DISCOUNT") &&
	 * object.getString("INVOICE DISCOUNT").toString().length() > 0) { double
	 * invoice = Double.parseDouble(object.getString("INVOICE DISCOUNT"));
	 * 
	 * if (invoice != 0) { if (!object.getString("printlanguageflag").equals("2")) {
	 * printlines2( (getAccurateText("INVOICE DISCOUNT", 20, 0) +
	 * getAccurateText(" : ", 3, 0) +
	 * getAccurateText(object.getString("INVOICE DISCOUNT"), 12, 0) +
	 * getAccurateText(" : ", 3, 0) + "*" +
	 * getAccurateText(ArabicTEXT.InvoiceDiscount, 15, 2) + "!"), 1, object, 1,
	 * args[0], 1, 1); } else { printlines2( (getAccurateText("INVOICE DISCOUNT",
	 * 65, 2) + getAccurateText(" : ", 3, 0) +
	 * getAccurateText(object.getString("INVOICE DISCOUNT"), 12, 2)), 1, object, 1,
	 * args[0], 1, 1); } } } printlines2( (getAccurateText("NET INVOICE AMOUNT", 20,
	 * 0) + getAccurateText(" : ", 3, 0) +
	 * getAccurateText(object.getString("NET SALES"), 12, 0) +
	 * getAccurateText(" : ", 3, 0) + "*" +
	 * getAccurateText(ArabicTEXT.NetInvoiceAmount, 20, 2) + "!"), 1, object, 1,
	 * args[0], 1, 1);
	 * 
	 * } else {
	 * 
	 ...
	 * 
	 * 
	 * printlines2( (getAccurateText("TOTAL GOOD RETURN", 65, 2) +
	 * getAccurateText(" : ", 3, 0) + getAccurateText(strAmnt, 12, 2)), 1, object,
	 * 1, args[0], 1, 1);
	 * 
	 * }else if (object.has("TOTAL GOOD RETURN")) { printlines2(
	 * (getAccurateText("TOTAL GOOD RETURN", 65, 2) + getAccurateText(" : ", 3, 0) +
	 * getAccurateText(object.getString("TOTAL GOOD RETURN"), 12, 2)), 1, object, 1,
	 * args[0], 1, 1);
	 * 
	 * } if (object.has("TOTAL BAD RETURN")) { printlines2(
	 * (getAccurateText("TOTAL BAD RETURN", 65, 2) + getAccurateText(" : ", 3, 0) +
	 * getAccurateText(object.getString("TOTAL BAD RETURN"), 12, 2)), 1, object, 1,
	 * args[0], 1, 1);
	 * 
	 * } if (object.has("INVOICE DISCOUNT") &&
	 * object.getString("INVOICE DISCOUNT").toString().length() > 0) { double
	 ...
	 * 65, 2) + getAccurateText(" : ", 3, 0) +
	 * getAccurateText(object.getString("INVOICE DISCOUNT"), 12, 2)), 1, object, 1,
	 * args[0], 1, 1); } } } outStream.write(BoldOn);
	 * 
	 * printlines2((getAccurateText("NET INVOICE AMOUNT", 65, 2) +
	 * getAccurateText(" : ", 3, 0) + getAccurateText(object.getString("NET SALES"),
	 * 12, 2)), 1, object, 1, args[0], 1, 1);
	 * 
	 * outStream.write(BoldOff);
	 * 
	 * }
	 * 
	 * 
	 * printlines2("", 1, object, 1, args[0], 1, 1); if (object.has("PaymentType")
	 * && Integer.parseInt(object.getString("PaymentType")) < 2) { // position =
	 * position + 60; outStream.write(BoldOn); if
	 * (!object.getString("LANG").equals("en")) {
	 * 
	 * printlines2(getAccurateText("*" + ArabicTEXT.PaymentDetails + "!", 80, 1), 2,
	 * object, 1, args[0], 1, 1); } else {
	 * printlines2(getAccurateText("PAYMENT DETAILS", 80, 1), 2, object, 1, args[0],
	 * 1, 1);
	 * 
	 * } outStream.write(BoldOff); // lp.newLine(2);
	 * 
	 * printlines2(printSeprator(), 1, object, 1, args[0], 1, 1);
	 * 
	 * JSONArray jCheques = object.has("Cheque") ? object.getJSONArray("Cheque") :
	 * null; JSONObject jCash = object.has("Cash") ? object.getJSONObject("Cash") :
	 * null; int paymenttype = Integer.parseInt(object.getString("ptype"));
	 * 
	 * switch (paymenttype) { case 0:
	 * 
	 * outStream.write(BoldOn); if (!object.getString("LANG").equals("en")) {
	 * 
	 * printlines2(getAccurateText("*" + ArabicTEXT.Cash + "! :" +
	 * jCash.getString("Amount"), 80, 1), 2, object, 1, args[0], 1, 1); } else {
	 * printlines2(getAccurateText("CASH:" + jCash.getString("Amount"), 80, 1), 2,
	 * object, 1, args[0], 1, 1); }
	 * 
	 * outStream.write(BoldOff); break; case 1: outStream.write(BoldOn); //
	 * lp.write("CHEQUE"); printlines2(getAccurateText("CHEQUE", 80, 1), 2, object,
	 * 1, args[0], 1, 1); outStream.write(BoldOff); // lp.newLine(2); printlines2(
	 * (getAccurateText("Cheque Date:", 15, 0) + getAccurateText("Cheque No:", 15,
	 * 0) + getAccurateText("Bank:", 35, 0) + getAccurateText("Amount:", 15, 2)), 1,
	 * object, 1, args[0], 1, 1); printlines2(printSeprator(), 1, object, 1,
	 * args[0], 1, 1);
	 * 
	 * for (int j = 0; j < jCheques.length(); j++) { JSONObject jChequeDetails =
	 * jCheques.getJSONObject(j); printlines2(
	 * (getAccurateText(jChequeDetails.getString("Cheque Date"), 15, 0) +
	 * getAccurateText(jChequeDetails.getString("Cheque No"), 15, 0) +
	 * getAccurateText(!object.getString("LANG").equals("en")?"*"+jChequeDetails.
	 * getString("Bank")+"!":jChequeDetails.getString("Bank"), 35, 0) +
	 * getAccurateText(jChequeDetails.getString("Amount"), 15, 2)), 1, object, 1,
	 * args[0], 1, 1);
	 * 
	 * } // lp.writeLine(printSeprator()); printlines2(printSeprator(), 1, object,
	 * 1, args[0], 1, 1); printlines2("", 1, object, 1, args[0], 1, 1); //
	 * lp.newLine(2); break; case 2: outStream.write(BoldOn); if
	 * (!object.getString("LANG").equals("en")) {
	 * 
	 * printlines2(getAccurateText("*" + ArabicTEXT.Cash + "! :" +
	 * jCash.getString("Amount"), 80, 1), 2, object, 1, args[0], 1, 1); } else {
	 ...
	 * printlines2("", 1, object, 1, args[0], 1, 1);
	 * printlines2(getAccurateText("CHEQUE", 80, 1), 2, object, 1, args[0], 1, 1);
	 * outStream.write(BoldOff); // lp.newLine(2); printlines2(
	 * (getAccurateText("Cheque Date:", 15, 0) + getAccurateText("Cheque No:", 15,
	 * 0) + getAccurateText("Bank:", 35, 0) + getAccurateText("Amount:", 15, 2)), 1,
	 * object, 1, args[0], 1, 1); printlines2(printSeprator(), 1, object, 1,
	 * args[0], 1, 1);
	 ...
	 * 1, args[0], 1, 1);
	 * 
	 * printlines2("", 1, object, 1, args[0], 1, 1); outStream.write(BoldOff);
	 * 
	 * break; default: break; } }
	 * 
	 * 
	 * if(object.has("crates")){
	 * printlines2(getAccurateText("Crates Delivered : "+object.getString(
	 * "cratesDelivered"), 40, 0) + getAccurateText("Crates Picked Up : "
	 * +object.getString("cratesPikedUp"), 40, 0), 2, object, 1, args[0], 1, 1); }
	 * 
	 * 
	 * if (object.getString("comments").toString().length() > 0) { if
	 * (object.getString("LANG").equals("en")) { printlines2("Comments:" +
	 * object.getString("comments"), 2, object, 1, args[0], 1, 1); } else {
	 * printlines2("Comments:*" + object.getString("comments") + "!", 2, object, 1,
	 * args[0], 1, 1);
	 * 
	 * } }
	 * 
	 * if (object.has("DeliveryNumber") &&
	 * object.getString("DeliveryNumber").toString().length() > 0) {
	 * 
	 * printlines2("Delivery Number : " + object.getString("DeliveryNumber"), 2,
	 * object, 1, args[0], 1, 1);
	 * 
	 * }
	 * 
	 ...
	 * 
	 * String
	 * amountpaid=object.has("amountpaid")?object.getString("amountpaid"):"0";
	 * if(amountpaid!=null && !TextUtils.isEmpty(amountpaid) &&
	 * Double.parseDouble(amountpaid)==0) { String
	 * invoiceamount=object.getString("NET SALES"); double netsales=0;
	 * if(invoiceamount!=null && !TextUtils.isEmpty(invoiceamount)){
	 * netsales=Double.parseDouble(invoiceamount); } /* String
	 * salesmemo=object.has("salesmemo")?object.getString("salesmemo"):"";
	 * if(salesmemo!=null && !TextUtils.isEmpty(salesmemo) && salesmemo.length()>0
	 * && netsales>0){ String[] tokens=salesmemo.split("[\\#]"); if(tokens!=null &&
	 * tokens.length>0){
	 * 
	 * 
	 * count=count+2; printlines2("", 1, object, 1, args[0], 1, 1); String
	 * token1=tokens[0].replace("***", object.getString("cname"));
	 * printlines2(token1, 1, object, 1, args[0], 1, 1); printlines2(tokens[1], 1,
	 * object, 1, args[0], 1, 1); printlines2(tokens[2], 1, object, 1, args[0], 1,
	 * 1); }
	 * 
	 * }
	 */
	 ...
	 * 
	 * 
	 * Log.e("CountNOW", "" + count); printlines2("", 3, object, 1, args[0], 1, 1);
	 * if (object.getString("printlanguageflag").equals("2")) {
	 * 
	 * if (object.has("TOTAL BAD RETURN") || object.has("totalFreeQty")) {
	 * 
	 * printlines2(getAccurateText("______________________   ", 26,
	 * 1)+getAccurateText("______________________   ", 25, 1) +
	 * getAccurateText("______________________   ", 25, 1), 2, object, 1, args[0],
	 * 1, 1);
	 * 
	 * printlines2(getAccurateText("SALESMAN", 26, 1)
	 * +getAccurateText("RECEIVED BY", 25, 1)+ getAccurateText("SIGN./STAMP", 25,
	 * 1), 2, object, 1, args[0], 1, 1); } else {
	 * printlines2(getAccurateText("______________________   ", 26,
	 * 1)+getAccurateText("______________________   ", 25, 1) +
	 * getAccurateText("______________________   ", 25, 1), 2, object, 1, args[0],
	 * 1, 1);
	 ...
	 * printlines2(getAccurateText("SALESMAN", 26, 1)
	 * +getAccurateText("RECEIVED BY", 25, 1)+ getAccurateText("SIGN./STAMP", 25,
	 * 1), 2, object, 1, args[0], 1, 1); }
	 * 
	 * } else {
	 * 
	 * if (object.has("TOTAL BAD RETURN") || object.has("totalFreeQty")) {
	 * 
	 * printlines2(getAccurateText("______________________   ", 23,
	 * 1)+getAccurateText("______________________   ", 27, 1) +
	 * getAccurateText("______________________   ", 28, 1), 2, object, 1, args[0],
	 * 1, 1);
	 * 
	 * printlines2(getAccurateText("SALESMAN *" + ArabicTEXT.SALESMAN + "!", 23,
	 * 1)+getAccurateText("RECEIVED BY *" + ArabicTEXT.Customer+ "!", 27, 1) +
	 * getAccurateText("SIGNATURE/STAMP", 28, 1), 2, object, 1, args[0], 1, 1);
	 * 
	 * 
	 * outStream.write(CompressOff); } else {
	 * printlines2(getAccurateText("______________________   ", 23,
	 * 1)+getAccurateText("______________________   ", 27, 1) +
	 * getAccurateText("______________________   ", 28, 1), 2, object, 1, args[0],
	 * 1, 1);
	 ...
	 * }
	 * 
	 * String copyStatus = ""; if
	 * (object.getString("printstatus").equals("DUPLICATE COPY")) { if
	 * (object.getString("printlanguageflag").equals("2")) { copyStatus =
	 * getAccurateText(object.getString("printstatus"), 80, 1) + " "; } else {
	 * copyStatus = getAccurateText( object.getString("printstatus") + "  *" +
	 * ArabicTEXT.DuplicateCopy + "", 80, 1) + "!"; } } else if
	 * (object.getString("printstatus").equals("ORIGINAL COPY")) { if
	 * (object.getString("printlanguageflag").equals("2")) { copyStatus =
	 * getAccurateText(object.getString("printstatus"), 80, 1) + " "; } else {
	 * copyStatus = getAccurateText( object.getString("printstatus") + "  *" +
	 * ArabicTEXT.OriginalCopy + "!", 80, 1); }
	 * 
	 * } else { if (object.getString("printlanguageflag").equals("2")) { copyStatus
	 * = getAccurateText(object.getString("printstatus"), 80, 1); } else {
	 * copyStatus = getAccurateText(object.getString("printstatus") + "  *" +
	 * ArabicTEXT.DraftCopy + "!", 80, 1); }
	 * 
	 * }
	 * 
	 * printlines2(copyStatus, 2, object, 2, args[0], 1, 1);
		...
		try {
			...
			if (object.has("ORDER DISCOUNT") && object.getString("ORDER DISCOUNT").toString().length() > 0) {
				...
				if (invoice != 0) {
					...
					printlines2((getAccurateText("ORDER DISCOUNT", 20, 0) + getAccurateText(" : ", 3, 0)
							+ getAccurateText(object.getString("ORDER DISCOUNT"), 12, 0) + getAccurateText(" : ", 3, 0)
							+ "*" + getAccurateText(ArabicTEXT.OrderDiscount, 15, 2) + "!"), 1, object, 1, args[0], 1,
							1);
				}
			}
	...
	void parseCollectionResponse(final JSONObject object, final String... args) {
		...
		try {
			...
			/* For Testing */
		/*	JSONObject jTOBject1 = object.getJSONObject("BALANCE");*/
			...
			/*	if (jTOBject1.has(headers.getString(i))) {
					strTotal = strTotal + getAccurateText(jTOBject1.getString(headers.getString(i).toString()),
							hashValues.get(headers.getString(i).toString()) + MAXLEngth,
							hashPositions.get(headers.getString(i).toString()));
				} else {

					strTotal = strTotal + getAccurateText(headers.getString(i).equals("Due Amount") ? "BALANCE" : "",
							hashValues.get(headers.getString(i)) + MAXLEngth, 1);
				}
			...
			//	printlines2(getAccurateText("PAYMENT DETAILS", 80, 1), 2, object, 1, args[0], 2, 2);
				printlines2(getAccurateText("PAYMENTS", 80, 1), 2, object, 1, args[0], 2, 2);
			//	printlines2(getAccurateText("Balance Due: " + object.getString("balancedue"), 80, 1), 2, object, 1, args[0], 2, 2);
			}
			...
			case 0:
				...
				if (!object.getString("LANG").equals("en")) {
			        ...
			        //Collection Report Balance due 14-04-2022
					double balance = Double.parseDouble(object.getString("balancedue"));
					double dblamount = Double.parseDouble(jCash.getString("Amount"));
					double newbalance = balance-dblamount;
					
					
					
					printlines2(getAccurateText("CASH PAID:" + jCash.getString("Amount"), 80, 1), 1, object, 1, args[0], 2,
							2);
					
					printlines2(getAccurateText("New Balance Due: " + newbalance, 80, 1), 2, object, 1, args[0], 2, 2);
				}
		...
		try {
			...
			for (int i = 0; i < jData.length(); i++) {
				...
				for (int j = 0; j < jArr.length(); j++) {
					String itemDescrion = jArr.getString(j);
					/*
					 * if (j == 9) { itemDescrion = "       " + jArr.getString(j) + ""; }
					 */
	...
	void printSalesSummaryReport(final JSONObject object, final String... args) {
		...
		try {
			...
			printlines1(getAccurateText("GRAND TOTAL", 40, 1) + getAccurateText(" ", 27, 1)
					+ getAccurateText(object.getString("GRANDTOT"), 10, 1)
					+ getAccurateText(object.getString("RETTOT"), 10, 2) + getAccurateText(" ", 2, 1)
					+ getAccurateText(object.getString("BADTOT"), 10, 2) + getAccurateText(" ", 2, 1)
					+ getAccurateText(object.getString("DISTOT"), 10, 2) + getAccurateText(" ", 3, 1)
					+ getAccurateText(object.getString("INVTOT"), 10, 2) + getAccurateText(" ", 2, 1)
					+ getAccurateText(object.getString("AMTTOT"), 10, 2), 2, object, 1, args[0], 5);
	...
	void printRouteSummaryReport(final JSONObject object, final String... args) {
		...
		try {
			...
			outStream.write(BoldOn);
			printlines2(
					getAccurateText("", 20, 0) + getAccurateText("VALUE", 15, 1) + getAccurateText("QUANTITY", 10, 2),
					1, object, 1, args[0], 6, 6);
			outStream.write(BoldOff);
			...
			// org line
			/*
			 * printlines2(getAccurateText("LOADED", 20, 0) +
			 * getAccurateText(object.getString("Loaded"), 10, 2), 1, object, 1, args[0], 6,
			 * 6);
			 */
			...
			/*
			 * printlines2(getAccurateText("LOADQTY", 40, 0) +
			 * getAccurateText(object.getString("LoadQty"), 20, 2), 1, object, 1, args[0],
			 * 6, 6);
			 */

			printlines2(getAccurateText("TRANSFERED IN", 20, 0) + getAccurateText(object.getString("Transferin"), 10, 2)
			...
			printlines2(
					getAccurateText("TRANSFERED OUT", 20, 0) + getAccurateText(object.getString("Transferout"), 10, 2)
							+ getAccurateText(object.getString("transferoutqty"), 15, 2),
					1, object, 1, args[0], 6, 6);
			...
			printlines2(getAccurateText("FRESH UNLOAD", 20, 0) + getAccurateText(object.getString("freshunload"), 10, 2)
			...
			printlines2(
					getAccurateText("TRUCK DAMAGES", 20, 0) + getAccurateText(object.getString("truckdamage"), 10, 2)
							+ getAccurateText(object.getString("truckdamageqty"), 15, 2),
					1, object, 1, args[0], 6, 6);
			...
			printlines2(getAccurateText("    CASH SALES", 20, 0) + getAccurateText(object.getString("cashsales"), 10, 2)
					+ getAccurateText(object.getString("cashsalesqty"), 15, 2), 1, object, 1, args[0], 6, 6);

			printlines2(
					getAccurateText("    CREDIT SALES", 20, 0) + getAccurateText(object.getString("creditsales"), 10, 2)
							+ getAccurateText(object.getString("creditsalesqty"), 15, 2),
					1, object, 1, args[0], 6, 6);
	...
	private void printlines2(String data, int ln, JSONObject object, int sts, String adr, int tran, int tp)
		...
		/*
		 * int pln=0; if(tran == 1 ) { pln=39; }else { pln=41; }
		 */
		int pln = 39; // 39 // Total Page Length (print area in one page)// Suresh 
		...
		if (sts == 2 && count != 0) {
			...
			// lnno = lnno + endln;
			/*
			 * if(Paireddevicename.equals("6824")) { endln = 10; }
			 */
			// endln = 9;
			...
			// outStream.write(CarriageReturn); // sujee commented for sales space for next
			// page 13/06.2018
			// outStream.write(NewLine);
			// outStream.write(NewLine);
			// outStream.write(NewLine);
		...
		if (!isEnd) {
			...
			if (count > pln) {
				...
				try {
					// sujee commented 17/08/2020
					// line(cnln);
					line(15);// Skip to next page set the the number of lines to skip to reach the header
					// outStream.write(CompressOff);
					if (tran == 2) {
	...
	/*
	 * private void headerTaxprint(JSONObject object, int type, String... args)
	 * throws JSONException { try {
	 * 
	 * 
	 * outStream.write(BoldOn); outStream.write(DoubleWideOn);
	 * printArabic(getAccurateText(object.getString("companyname"), 40, 1));
	 * outStream.write(DoubleWideOff); outStream.write(NewLine);
	 * printArabic(getAccurateText(object.getString("companyaddress"), 80, 1));
	 * 
	 * printheaders(getAccurateText("TAX CARD NO: " + object.getString("taxcodeno")
	 * + " , CARD NO: ("+object.getString("companycrno")+")", 80, 1), false, 1); if
	 * (object.has("companytaxregistrationnumber")){
	 * 
	 * printArabic(getAccurateText("VATIN: "+object.getString(
	 * "companytaxregistrationnumber"), 80, 1));
	 * 
	 * }
	 * 
	 * 
	 * if (type == 1) { outStream.write(BoldOn); outStream.write(DoubleWideOn); if
	 * (!object.getString("LANG").equals("en")) {
	 * 
	 * if (object.getString("invoicepaymentterms").contains("2")) {
	 ...
	 * outStream.write(NewLine);
	 * 
	 * printArabic(getAccurateText( object.getString("INVTYPE") +"INVOICE NUMBER " +
	 * object.getString("invoicenumber"), 40, 1));
	 * 
	 * 
	 ...
	 * 40, 1)).getBytes());
	 * 
	 * outStream.write(NewLine); outStream.write((getAccurateText( "INVOICE NUMBER "
	 ...
	 * 
	 * 
	 * outStream.write((getAccurateText( object.getString("INVTYPE") +
	 * "INVOICE NUMBER: " + object.getString("invoicenumber"), 40, 1)).getBytes());
	 * 
	 * outStream.write(DoubleWideOff); outStream.write(BoldOff); }
	 * 
	 * outStream.write(NewLine); outStream.write(NewLine); outStream.write(BoldOn);
	 * 
	 * }else if (type == 2) {
	 * 
	 * outStream.write(BoldOn); outStream.write(DoubleWideOn); if
	 * (!object.getString("LANG").equals("en")) {
	 * 
	 * if (object.getString("invoicepaymentterms").contains("2")) {
	 * printArabic(getAccurateText( "*" + ArabicTEXT.Creditinvoice + "! ", 40, 1));
	 * outStream.write(NewLine); printArabic(getAccurateText( "ORDER NO " +
	 * object.getString("invoicenumber"), 40, 1));
	 * 
	 * } else if (object.getString("invoicepaymentterms").contains("0") ||
	 * object.getString("invoicepaymentterms").contains("1")) {
	 ...
	 * } else {
	 * 
	 * outStream.write((getAccurateText(object.getString("INVOICETYPE"), 40,
	 * 1)).getBytes());
	 * 
	 * outStream.write(NewLine); outStream.write((getAccurateText( "ORDER NO " +
	 * object.getString("invoicenumber"), 40, 1)).getBytes()); }
	 * 
	 * } else { outStream.write((getAccurateText(object.getString("INVOICETYPE"),
	 * 40, 1)).getBytes()); outStream.write(NewLine);
	 ...
	 * outStream.write(NewLine);
	 * 
	 * //outStream.write(BoldOff); outStream.write(NewLine);
	 * outStream.write(BoldOn); outStream.write(DoubleWideOn);
	 * outStream.write((getAccurateText( "ORDER NO " +
	 * object.getString("invoicenumber"), 40, 1)).getBytes());
	 * outStream.write(DoubleWideOff); outStream.write(BoldOff); }
	 * 
	 * outStream.write(NewLine); outStream.write(NewLine); outStream.write(BoldOn);
	 ...
	 * } catch (Exception e) { e.printStackTrace(); }
	 * 
	 * outStream.write(NewLine); //------------- END ----------------------
	 * 
	 * 
	 * printArabic(getAccurateText("" + object.getString("ADDRESS1"), 60, 1) +
	 * getAccurateText("", 10, 0) + getAccurateText( "INVOICE DATE:" +
	 * object.getString("DOC DATE") + " (" + object.getString("TIME") + ")" , 25,
	 * 2));
	 * 
	 * 
	 * 
	 ...
	 * "ROUTE : " + object.getString("ROUTE"), 40, 2)); outStream.write(NewLine);
	 * 
	 * }
	 * 
	 ...
	 * 
	 * printArabic(getAccurateText("TAX CARD NO: " + object.getString("TAXCARDNO"),
	 * 40, 0) ); outStream.write(NewLine);
	 * 
	 * 
	 * 
	...
	private void headerTaxprint(JSONObject object, int type, String... args) throws JSONException {
		try {
			...
			outStream.write(BoldOn);
			outStream.write(DoubleWideOn);
			printArabic(getAccurateText("", 5, 1) + getAccurateText(object.getString("companyname"), 30, 1));
			...
			if (type == 1) {
				...
				if (!object.getString("LANG").equals("en")) {
					...
					if (object.getString("invoicepaymentterms").contains("2")) {
						...
						outStream.write(NewLine);
						printArabic(getAccurateText(
								"INVOICE NUMBER : " + object.getString("invoicenumber"), 40, 1));
						...
						outStream.write(BoldOn);
						outStream.write((getAccurateText(object.getString("INVOICETYPE") +  object.getString("invoicenumber"), 40, 1)).getBytes());
						//outStream.write(NewLine);
						//printheaders(getAccurateText("",10, 1) +  getAccurateText("VATIN: "+object.getString("companytaxregistrationnumber"), 30, 0),true,2);
						//outStream.write(NewLine);
					...
					//outStream.write(NewLine);
					//outStream.write(BoldOn);
				//	printheaders(getAccurateText("", 30, 1) +  getAccurateText("VATIN: "+object.getString("companytaxregistrationnumber"), 30, 0) ,true,2);
					//outStream.write(BoldOff);
					//outStream.write(BoldOn);
					//outStream.write(NewLine);
					//outStream.write(DoubleWideOn);
				...
				if (!object.getString("LANG").equals("en")) {
					...
					if (object.getString("invoicepaymentterms").contains("2")) {
						...
						//outStream.write(NewLine);
						//printheaders(getAccurateText("", 45, 1) +  getAccurateText("VATIN: "+object.getString("companytaxregistrationnumber"), 30, 0) 
						//+getAccurateText("*" + ArabicTEXT.taxdetail + "! ", 10, 2),true,2);
						outStream.write(NewLine);
						outStream.write((getAccurateText(
				...
				try {
					// org line sujee commented not to print cust code & address in 1 line 11/06/2018
					/*String[] parts = object.getString("CUSTOMER").split("\\-");
					outStream.write(("CUSTOMER: " + parts[0]).getBytes());*/
					
					// sujee added 11/06/2018 -------- Start ----------------
					//outStream.write((getAccurateText("CUSTOMER : " +object.getString("CUSTOMERID"), 25, 1)  ).getBytes());
					printArabic(getAccurateText("CUSTOMER : " + object.getString("CUSTOMERID"), 50, 0) + getAccurateText(
							"INVOICE DATE:" +  object.getString("DOC DATE") + " (" + object.getString("TIME") + ")"  , 30, 2));
					outStream.write(NewLine);
					//outStream.write((getAccurateText(object.getString("CUSTOMERNAME"), 45, 1)).getBytes());
					outStream.write(("CUSTOMER: " + object.getString("CUSTOMERNAME")).getBytes());
				/*	printArabic(getAccurateText(object.getString("CUSTOMERNAME"), 50, 0) + 
										getAccurateText("TYPE:" + object.getString("INVTYPE") , 20, 2));
					outStream.write(NewLine);
					//------------- END ----------------------
					/*printArabic("          *" + parts[1] + "!");
					outStream.write(NewLine);*/
					...
					// org line sujee not to print customer address 07/08/2018
			/*	printArabic(getAccurateText("ADDRESS : " + object.getString("ADDRESS"), 40, 0) + getAccurateText(
							"INVOICE DATE:" + object.getString("DOC DATE") + " (" + object.getString("TIME") + ")", 40, 2));
					*/
				/*	printArabic( getAccurateText(
							"DATE:" + object.getString("DOC DATE") + " (" + object.getString("TIME") + ")", 80, 2));
					*/
				...
				if (object.has("taxregistrationnumber")){
					...
					printArabic(getAccurateText("VATIN : " + object.getString("taxregistrationnumber"), 40, 0) + getAccurateText(
							"TYPE : " + object.getString("INVTYPE"), 40, 1));
					outStream.write(NewLine);
					
				}
				printArabic(getAccurateText("ROUTE: " + object.getString("ROUTE") + "-TRIP ID:"+object.getString("TourID") , 50, 0) + getAccurateText(
						"SUPPLY DATE:" + object.getString("DOC DATE") + "(" + object.getString("TIME") + ")", 30, 1));

				outStream.write(NewLine);
				
				printArabic(getAccurateText("SALESMAN : " +object.getString("SALESMAN") + "-" + object.getString("CONTACTNO"), 40, 0) +
						getAccurateText("SUPERVISOR : " + object.getString("supervisorname") + "-" + object.getString("supervisorno"), 40, 1));
				
				//printArabic(getAccurateText("SUPERVISOR : " + object.getString("supervisorname") + "-" + object.getString("supervisorno"), 40, 0));
				
			//	printArabic(getAccurateText("ADVANCE PAYMENT : NOT AVAILABLE", 40, 0));
				printArabic(getAccurateText("ADVANCE PAYMENT : NA" , 35, 0) + getAccurateText("PAYMENT DATE: AS PER AGREEMENT"  , 45, 2));
				outStream.write(NewLine);
	...
	private void headerTaxprintBackup(JSONObject object, int type, String... args) throws JSONException {
		try {
...
//			}
			...
			if (type == 1) {
				...
				outStream.write(NewLine);
				outStream.write(
						(getAccurateText(object.getString("INVTYPE") + "# " + object.getString("invoicenumber"), 40, 1))
								.getBytes());
				outStream.write(BoldOff);
				outStream.write(DoubleWideOff);
			...
			printArabic(getAccurateText("" + object.getString("ADDRESS1"), 40, 0) + getAccurateText(
					"INVOICE DATE:" + object.getString("DOC DATE") + " (" + object.getString("TIME") + ")", 40, 2));
			...
			printArabic(getAccurateText("" + object.getString("ADDRESS3"), 40, 0)
					+ getAccurateText("SALESMAN:" + object.getString("SALESMAN"), 40, 2));
			outStream.write(NewLine);

			printArabic(getAccurateText(object.getString("TAXCARDNO"), 40, 0) + getAccurateText(
					"SUPERVISOR :" + object.getString("supervisorname") + " (" + object.getString("supervisorno") + ")",
					40, 2));
			outStream.write(NewLine);

			// printArabic(getAccurateText("" + object.getString("CRNO"), 40, 0) );
			printArabic(getAccurateText(object.getString("CRNO"), 40, 0)
					+ getAccurateText("ROUTE :" + object.getString("ROUTE"), 30, 2));
			outStream.write(NewLine);

			printArabic(getAccurateText(object.getString("CUSTOMERPHONE"), 60, 0));
			outStream.write(NewLine);
			printArabic(getAccurateText(object.getString("EMAIL"), 40, 0));
			// outStream.write((getAccurateText(object.getString("EMAIL"), 40,
			// 1)).getBytes());
			...
			// printArabic(getAccurateText("", 40, 0) + getAccurateText("ADVANCE PAYMENT:
			// N/A" , 40, 2));
			// outStream.write(NewLine);
		...
		try {
			...
			if (type == 1) {
				...
				if (!object.getString("LANG").equals("en")) {
					...
					if (object.getString("invoicepaymentterms").contains("2")) {
						...
						if (object.getString("DELIVERYFLAG").equals("1")) {
							printheaders(
									(getAccurateText("DELIVERY NOTE : " + object.getString("invoicenumber"), 40, 1)),
									false, 2);
						} else {
					...
					if (object.getString("DELIVERYFLAG").equals("1")) {
						printheaders((getAccurateText("DELIVERY NOTE : " + object.getString("invoicenumber"), 40, 1)),
								false, 2);
					} else {
						printheaders((getAccurateText(object.getString("INVOICETYPE"), 40, 1)), false, 2);
					}
				...
				try {
					...
					// outStream.write(NewLine);
					/*
					 * outStream.write((getAccurateText("CUSTOMER : "
					 * +object.getString("CUSTOMERID"), 10, 1)).getBytes());
					 * outStream.write(NewLine);
					 */
					outStream.write(BoldOn);
			...
			outStream.write(NewLine);
			// outStream.write(BoldOn);
			printheaders(getAccurateText("ROUTE: " + object.getString("ROUTE"), 40, 0) + getAccurateText(
					"INVOICE DATE:" + object.getString("DOC DATE") + " (" + object.getString("TIME") + ")", 40, 2),
					true, 1);
			outStream.write(NewLine);
			...
			if (object.getString("currname").equals("OMR")) {
				outStream.write(NewLine);
				printheaders(
						getAccurateText("SUPERVISOR NAME:" + object.getString("supervisorname"), 40, 0)
			...
			if (type == 3 || type == 5 || type == 6 || type == 4 || type == 7) {
				...
				// sujee commented 19/06/2019
				// printheaders((getAccurateText("TOUR ID:" + object.getString("TourID"), 80,
				// 0)), false, 1);
	...
	private void headerinvprint(JSONObject object, int invtype) throws JSONException {
		try {
			 ...
			 * "TAX CARD NO: " + object.getString("taxcodeno"), 20, 0) +
			 * getAccurateText("CARD NO: " + object.getString("companycrno"), 20,
			 * 0)).getBytes());
			 * 
			 * printheaders( getAccurateText( "TAX CARD NO:" + object.getString("taxcodeno")
			 * + " , CARD NO: (" + object.getString("companycrno") + " , VATIN: ("+ vatno +
			 * ")", 40, 1), true, 1);
			 * 
			 * 
			 * printheaders(getAccurateText("TAX CARD NO: " + object.getString("taxcodeno")
			 ...
			 * (object.has("companytaxregistrationnumber")){
			 * printArabic(getAccurateText("VATIN: "+object.getString(
			 * "companytaxregistrationnumber"), 80, 1)); outStream.write(NewLine);
			 * //outStream.write(NewLine); }
			 */
			...
			outStream.write(DoubleWideOn);
			if (invtype == 1) {
				printheaders(getAccurateText("LOAD SUMMARY - LOAD: " + object.getString("Load Number"), 40, 1), false,
						1);
			} else if (invtype == 2) {
